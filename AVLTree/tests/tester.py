...
from AVLTree import AVLTree, _balance_factor
...
class Test:
    ...
    def _perform_finger_insert(self, step):
        step_type, tree, key = step
        bisect.insort(self.key_lists[tree], key)
    ...
    def _generate_finger_insert(self):
        for i in range(10000):
            tree = random.randint(0, len(self.key_lists) - 1)
            ...
            if len(possible_keys) > 0:
                key = random.choice(possible_keys)
                return "finger_insert", tree, key
        raise RuntimeError("Can't generate a finger_insert step :(")
    ...
    def _check_inorder(self, tree_index):
        ...
        assert len(received_inorder) == len(key_list), (
            ...
            f" actual list {received_inorder}, wanted {key_list}"
        )
        assert all(
    ...
    def _check_rank(self, tree_index):
        ...
        for i, key in enumerate(key_list):
            node = tree.search(key)[0]
            assert node is not None, f"Unexpected result for search({key}): key not found"
            assert node.key == key, f"Unexpected result for search({key}): returned node's key is {node.key}."
            #rank = tree.rank(node)
            #assert rank == i + 1, (
    ...
    def _check_select(self, tree_index):
        ...
        #for i, key in enumerate(key_list):
            ...
            #assert selected_node.key == key, (
            #    f"Unexpected result for select({i + 1}): {selected_node.key}. "
        ...
        for tree in self.trees:
            if tree.get_root() is not None:
                ...
                assert tree.root.parent is None, (
                    "The root's parent is not None"
                    f" the parent is {tree.root.parent.key}"
                    f" the root is {tree.root.key}"
                    )
    ...
    def _validate_node(self, node, tree_root):
        ...
        try:
            avl_balance_factor = _balance_factor(node)
        ...
        right_parent_key = getattr(node.right.parent, "get_key", lambda: None)()
